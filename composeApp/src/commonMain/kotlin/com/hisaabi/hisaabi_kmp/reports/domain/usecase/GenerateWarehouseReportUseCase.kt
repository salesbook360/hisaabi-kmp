...
/**
 * Use case for generating warehouse reports from actual database
 * Similar to Stock Report but filtered by selected warehouse
 */
class GenerateWarehouseReportUseCase(
    ...
    suspend fun execute(filters: ReportFilters): ReportResult {
        ...
        val selectedWarehouseId = filters.selectedWarehouseId
            ?: throw IllegalStateException("Warehouse must be selected for Warehouse Report")
        
        // Get all active products
        val productEntities = productDao.getProductsByBusiness(businessSlug).first()
            .filter { it.status_id == 0 } // Only active products
        
        // Get product quantities filtered by selected warehouse (not summing across warehouses)
        val productQuantities = productQuantitiesDao.getQuantitiesByBusiness(businessSlug)
            .filter { it.warehouse_slug == selectedWarehouseId && it.product_slug != null }
        
        // Map quantities by product slug (only from selected warehouse)
        val quantitiesByProduct = productQuantities
            .associateBy { it.product_slug!! }
            .mapValues { (_, quantity) ->
                // Use quantity from selected warehouse only (not summing)
                AggregatedProductQuantity(
                    currentQuantity = quantity.current_quantity,
                    openingQuantity = quantity.opening_quantity,
                    minimumQuantity = quantity.minimum_quantity,
                    maximumQuantity = quantity.maximum_quantity
                )
            }
        
        // Filter products that have quantity in the selected warehouse
        val products = productEntities
            .map { entity -> entity.toDomainModel() }
            .filter { product -> quantitiesByProduct.containsKey(product.slug) }
        
        // Calculate stock stats for the selected warehouse
        ...
        val (columns, rows, summary) = when (filters.additionalFilter) {
            ...
            ReportAdditionalFilter.OUT_OF_STOCK -> {
                ...
                generateStockInOutReport(filters, products, quantitiesByProduct, currencySymbol, stockStats, businessSlug, selectedWarehouseId)
            }
        }
    ...
    private suspend fun generateStockInOutReport(
        ...
        val warehouseTransactions = transactions.filter { transaction ->
            transaction.ware_house_slug_from == warehouseSlug || transaction.ware_house_slug_to == warehouseSlug
        }
        
        // Get transaction details
        val transactionSlugs = warehouseTransactions.mapNotNull { it.slug }
        ...
        // Update stats from transactions (filtered by warehouse)
    ...
    private fun generateStockWorthReport(
        ...
        products.forEachIndexed { index, product ->
            val aggregatedQuantity = quantitiesByProduct[product.slug]
            val currentQuantity = aggregatedQuantity?.currentQuantity ?: 0.0
            val stockPurchaseWorth = product.avgPurchasePrice * currentQuantity
            ...
            rows.add(
                ReportRow(
                    ...
                    values = listOf(
                        ...
                        "$currencySymbol ${String.format("%,.2f", stockPurchaseWorth)}"
                    )
                )
            )
        }
        
        val totalStockPurchaseWorth = products.sumOf { product ->
            val quantity = quantitiesByProduct[product.slug]?.currentQuantity ?: 0.0
            product.avgPurchasePrice * quantity
        }
        ...
        val summary = ReportSummary(
            ...
            additionalInfo = mapOf(
                "Total Products" to stockStats.totalProducts.toString(),
                "Total Stock Purchase Worth" to "$currencySymbol ${String.format("%,.2f", totalStockPurchaseWorth)}"
            )
        )
    ...
    private fun generateOutOfStockReport(
        ...
        products.forEachIndexed { index, product ->
            ...
            // Only include products that are out of stock
